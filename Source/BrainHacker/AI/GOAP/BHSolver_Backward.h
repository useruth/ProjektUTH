...
#include "BHSolver_Backward.generated.h"

/**
 * Backward planning implementation.
 */
UCLASS()
class BRAINHACKER_API UBHSolver_Backward : public UBHSolver
{
	GENERATED_BODY()
...
private:
	 ...
	 */
	struct FBHNodeDesiredWorldState
	{
		FBHNodeDesiredWorldState() {};
		FBHNodeDesiredWorldState(const FBHWorldStateData& InDesiredState, const int32 InInstigatorActionIndex,
		...
		/** Relevant data. */
		FBHWorldStateData DesiredState;
	...
	struct FBHTreeNode
	{
		...
		TArray<FBHNodeDesiredWorldState> DesiredWorldStates;
	...
	TArray<UBHAction*> FindActionChangingWorldState(const FBHWorldStateData& DesiredWorldState) const;
	...
	TArray<FBHActionWithTargetData> BuildActionsPlanForPath(const TArray<FBHTreeNode>& KnownNodes, TArray<int32> Path) const;
	/**
	 ...
	 */
	TArray<int32> PrepareSolution(TArray<FBHTreeNode>& KnownNodes, TArray<int32> Solution) const;
	/**
	 ...
	 */
	bool IsActionEffectAlreadyActualForPlan(const TArray<FBHWorldStateData>& CurrentWorldState,
		const FBHWorldStateData& ActionEffect) const;
	/** Remove identical solutions from array. */
	void RemoveIdenticalSolutions(const TArray<FBHTreeNode>& KnownNodes, TArray<TArray<int32>>& PreparedSolutions) const;
	/** Return true if given solutions are identical (the same actions in the same order on the same targets). */
	bool AreSolutionsIdentical(const TArray<FBHTreeNode>& KnownNodes, const TArray<int32>& SolutionOne,
		const TArray<int32>& SolutionTwo) const;
	/** Return index of best solution (solution which actions summary cost is min). */
	int32 GetBestSolution(const TArray<FBHTreeNode>& KnownNodes, const TArray<TArray<int32>>& PreparedSolutions) const;
	/**
	 ...
	 */
	TArray<int32> GetAllNodesUsedForDesiredState(const TArray<FBHTreeNode>& KnownNodes, TArray<int32> Solution,
